...
Использование:
  1. Положи исходную иконку 1024x1024 как: aidiet-app/assets/icon-1024.png
...
SOURCE = Path(__file__).parent.parent / "aidiet-app" / "assets" / "icon-1024.png"
IOS_OUT = Path(__file__).parent.parent / "aidiet-app" / "ios" / "App" / "App" / "Assets.xcassets" / "AppIcon.appiconset"
ANDROID_OUT = Path(__file__).parent.parent / "aidiet-app" / "android" / "app" / "src" / "main" / "res"
...
def generate_icons():
    if not SOURCE.exists():
        ...
        print(f"   Создайте файл icon-1024.png в папке aidiet-app/assets/")
        return
    ...
    print(f"   Теперь запусти: cd aidiet-app && npx cap sync")
