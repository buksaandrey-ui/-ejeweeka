...
class PromptAssembler:
    """
    Универсальный и масштабируемый билдер промптов для генерации планов питания AIDiet.
...
"""
    ...
    def build_recipe_prompt(profile, missing_meals: list, target_kcal: float) -> str:
        ...
        prompt_parts = []
        prompt_parts.append(f"Ты шеф-повар и нутрициолог AIDiet. Твоя задача сгенерировать точные рецепты, КБЖУ, ингредиенты и пошаговые инструкции ТОЛЬКО для следующих {len(missing_meals)} блюд:\n" + ", ".join(missing_meals))
