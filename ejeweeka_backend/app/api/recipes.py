...
def get_or_generate_recipe_image(req: RecipeImageRequest, db: Session = Depends(get_db)):
    ...
    if req.excluded_allergens:
        ...
        allergen_clause = f' EXTREMELY IMPORTANT: DO NOT show, depict, or include ANY {excluded_str} under any circumstances. Ensure they are completely absent from the image.'
        negative_prompt_add = excluded_str

    prompt = f"Professional food photography of: {req.title}. Plated on a simple white ceramic plate. STRICTLY made ONLY using the following ingredients: {mapped_ingredients}. DO NOT include ANY other visible ingredients, garnishes, or side items that are not explicitly in this list.{allergen_clause} Overhead angle, natural daylight, photorealistic, 4k."
    ...
    try:
        ...
        try:
            ...
            db.rollback()
            print(f"Изображение для {ingredients_hash} уже кэшировано (concurrency).")
        except Exception as cache_e:
            db.rollback()
            print(f"Ошибка кэширования изображения: {cache_e}")
