...
def delete_account(request: TokenVerifyRequest, db: Session = Depends(get_db)):
    """Полностью удаляет анонимный профиль из БД в соответствии с Apple Guideline 5.1.1(v)."""
    user_id = _verify_token(request.token)
    if not user_id:
        raise HTTPException(status_code=401, detail="Token expired or invalid")
        
    user = db.query(User).filter(User.anonymous_uuid == user_id).first()
    if user:
        db.delete(user)
        db.commit()
        return {"status": "success", "message": "Account deleted securely."}
