...
import 'package:health_code/features/chat/data/chat_message_model.dart';
...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  Future<void> _send(String text) async {
    ...
    try {
      ...
      if (e.response?.statusCode == 503) {
        _addError('HC-ассистент временно перегружен. Попробуйте через минуту.');
      } else {
  ...
  Widget _lockBanner() => Container(
    ...
    child: Row(children: [
      ...
      ElevatedButton(
        onPressed: () => launchUrl(
          Uri.parse('https://t.me/healthcode_bot?start=subscribe'),
