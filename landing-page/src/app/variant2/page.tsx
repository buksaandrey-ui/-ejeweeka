...
const NodeDivider = () => (
  <div className="flex items-center justify-center gap-2 py-12 opacity-[0.15]">
    <svg width="8" height="8"><circle cx="4" cy="4" r="3" fill="#4C1D95"/></svg>
    <svg width="40" height="2"><line x1="0" y1="1" x2="40" y2="1" stroke="#4C1D95" strokeWidth="1"/></svg>
    <svg width="12" height="12"><circle cx="6" cy="6" r="5" fill="#4C1D95"/></svg>
    <svg width="40" height="2"><line x1="0" y1="1" x2="40" y2="1" stroke="#4C1D95" strokeWidth="1"/></svg>
    <svg width="6" height="6"><circle cx="3" cy="3" r="2.5" fill="#4C1D95"/></svg>
    <svg width="40" height="2"><line x1="0" y1="1" x2="40" y2="1" stroke="#4C1D95" strokeWidth="1"/></svg>
    <svg width="10" height="10"><circle cx="5" cy="5" r="4" fill="#4C1D95"/></svg>
  </div>
);
...
const featureSVGs: Record<string, React.ReactNode> = {
    ...
    <svg viewBox="0 0 120 120" className="w-full h-full">
      {[{x:60,y:20,r:5},{x:30,y:40,r:4},{x:90,y:35,r:4},{x:20,y:70,r:3},{x:50,y:55,r:5},{x:80,y:60,r:4},{x:40,y:85,r:3},{x:70,y:80,r:4},{x:100,y:75,r:3},{x:15,y:50,r:3},{x:105,y:50,r:3},{x:60,y:100,r:4}].map((n,i) => (
        <React.Fragment key={i}>
          <circle cx={n.x} cy={n.y} r={n.r} fill="#4C1D95" opacity={0.3 + Math.random()*0.7}/>
          {i > 0 && <line x1={n.x} y1={n.y} x2={[{x:60,y:20},{x:30,y:40},{x:90,y:35},{x:20,y:70},{x:50,y:55},{x:80,y:60},{x:40,y:85},{x:70,y:80},{x:100,y:75},{x:15,y:50},{x:105,y:50},{x:60,y:100}][(i-1)].x} y2={[{x:60,y:20},{x:30,y:40},{x:90,y:35},{x:20,y:70},{x:50,y:55},{x:80,y:60},{x:40,y:85},{x:70,y:80},{x:100,y:75},{x:15,y:50},{x:105,y:50},{x:60,y:100}][(i-1)].y} stroke="#4C1D95" strokeWidth="0.5" opacity="0.2"/>}
        </React.Fragment>
      ))}
    </svg>
  ),
  photo: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <rect x="30" y="25" width="60" height="75" rx="8" fill="none" stroke="#4C1D95" strokeWidth="1.5" opacity="0.6"/>
      <circle cx="60" cy="55" r="15" fill="none" stroke="#4C1D95" strokeWidth="1.5" opacity="0.8"/>
      <circle cx="60" cy="55" r="6" fill="#4C1D95" opacity="0.4"/>
      {[0,60,120,180,240,300].map((a,i) => <line key={i} x1={60+Math.cos(a*Math.PI/180)*20} y1={55+Math.sin(a*Math.PI/180)*20} x2={60+Math.cos(a*Math.PI/180)*30} y2={55+Math.sin(a*Math.PI/180)*30} stroke="#4C1D95" strokeWidth="0.8" opacity="0.3"/>)}
    </svg>
  ),
  vitamins: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <circle cx="35" cy="40" r="12" fill="none" stroke="#4C1D95" strokeWidth="1.5" opacity="0.8"/>
      <circle cx="85" cy="40" r="12" fill="none" stroke="#4C1D95" strokeWidth="1.5" opacity="0.8"/>
      ...
      <text x="35" y="44" textAnchor="middle" fill="#4C1D95" fontSize="8" fontWeight="bold">Fe</text>
      <text x="85" y="44" textAnchor="middle" fill="#4C1D95" fontSize="8" fontWeight="bold">Ca</text>
      <text x="60" y="89" textAnchor="middle" fill="#52B044" fontSize="8" fontWeight="bold">D3</text>
    </svg>
  ),
  geo: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      {[{x:30,y:30},{x:70,y:25},{x:100,y:45},{x:20,y:60},{x:55,y:55},{x:85,y:70},{x:35,y:90},{x:75,y:90}].map((p,i) => <circle key={i} cx={p.x} cy={p.y} r="3" fill="#4C1D95" opacity="0.2"/>)}
      <circle cx="55" cy="55" r="8" fill="none" stroke="#4C1D95" strokeWidth="1.5" opacity="0.9"/>
      <circle cx="55" cy="55" r="3" fill="#4C1D95" opacity="0.8"/>
      <circle cx="55" cy="55" r="18" fill="none" stroke="#4C1D95" strokeWidth="0.5" opacity="0.2"/>
      <circle cx="55" cy="55" r="30" fill="none" stroke="#4C1D95" strokeWidth="0.3" opacity="0.1"/>
    </svg>
  ),
  recalc: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      ...
      <polyline points="10,80 30,75 50,70 60,55 70,50 90,45 110,40" fill="none" stroke="#4C1D95" strokeWidth="1.5"/>
      <circle cx="60" cy="85" r="4" fill="rgba(255,255,255,0.3)"/>
      <circle cx="60" cy="55" r="4" fill="#4C1D95"/>
      <line x1="60" y1="82" x2="60" y2="58" stroke="#4C1D95" strokeWidth="0.8" opacity="0.5" strokeDasharray="2 1"/>
    </svg>
  ),
  shopping: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <circle cx="60" cy="60" r="35" fill="none" stroke="#4C1D95" strokeWidth="0.5" opacity="0.2"/>
      <circle cx="45" cy="45" r="15" fill="#4C1D95" opacity="0.1" stroke="#4C1D95" strokeWidth="0.8"/>
      <circle cx="75" cy="50" r="12" fill="#4C1D95" opacity="0.1" stroke="#4C1D95" strokeWidth="0.8"/>
      <circle cx="55" cy="75" r="13" fill="#4C1D95" opacity="0.1" stroke="#4C1D95" strokeWidth="0.8"/>
      <circle cx="45" cy="45" r="3" fill="#4C1D95" opacity="0.6"/><circle cx="75" cy="50" r="3" fill="#4C1D95" opacity="0.6"/><circle cx="55" cy="75" r="3" fill="#4C1D95" opacity="0.6"/>
    </svg>
  ),
  privacy: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <path d="M60 20 L95 40 L95 70 Q95 100 60 110 Q25 100 25 70 L25 40 Z" fill="none" stroke="#4C1D95" strokeWidth="1.5" opacity="0.4"/>
      <rect x="50" y="55" width="20" height="16" rx="3" fill="none" stroke="#4C1D95" strokeWidth="1.5" opacity="0.8"/>
      <path d="M54 55 L54 48 Q54 42 60 42 Q66 42 66 48 L66 55" fill="none" stroke="#4C1D95" strokeWidth="1.5" opacity="0.8"/>
      <circle cx="60" cy="63" r="2" fill="#4C1D95"/>
    </svg>
  ),
  fasting: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      ...
      <section id="features" className="py-24 px-6">
        <RevealSection>
          <div className="max-w-[1200px] mx-auto">
            <p className="text-center text-sm tracking-[0.25em] uppercase font-medium mb-4" style={{ color: "#4C1D95" }}>Уникальные возможности</p>
      ...
      <section className="py-24 px-6">
        <RevealSection>
          <div className="max-w-[1000px] mx-auto">
            <p className="text-center text-sm tracking-[0.25em] uppercase font-medium mb-4" style={{ color: "#4C1D95" }}>Один день</p>
