...
const NodeDivider = () => (
  <div className="flex items-center justify-center gap-2 py-12 opacity-[0.15]">
    <svg width="8" height="8"><circle cx="4" cy="4" r="3" fill="#F5922B"/></svg>
    <svg width="40" height="2"><line x1="0" y1="1" x2="40" y2="1" stroke="#F5922B" strokeWidth="1"/></svg>
    <svg width="12" height="12"><circle cx="6" cy="6" r="5" fill="#F5922B"/></svg>
    <svg width="40" height="2"><line x1="0" y1="1" x2="40" y2="1" stroke="#F5922B" strokeWidth="1"/></svg>
    <svg width="6" height="6"><circle cx="3" cy="3" r="2.5" fill="#F5922B"/></svg>
    <svg width="40" height="2"><line x1="0" y1="1" x2="40" y2="1" stroke="#F5922B" strokeWidth="1"/></svg>
    <svg width="10" height="10"><circle cx="5" cy="5" r="4" fill="#F5922B"/></svg>
  </div>
);
...
const featureSVGs: Record<string, React.ReactNode> = {
    ...
    <svg viewBox="0 0 120 120" className="w-full h-full">
      {[{x:60,y:20,r:5},{x:30,y:40,r:4},{x:90,y:35,r:4},{x:20,y:70,r:3},{x:50,y:55,r:5},{x:80,y:60,r:4},{x:40,y:85,r:3},{x:70,y:80,r:4},{x:100,y:75,r:3},{x:15,y:50,r:3},{x:105,y:50,r:3},{x:60,y:100,r:4}].map((n,i) => (
        <React.Fragment key={i}>
          <circle cx={n.x} cy={n.y} r={n.r} fill="#F5922B" opacity={0.3 + Math.random()*0.7}/>
          {i > 0 && <line x1={n.x} y1={n.y} x2={[{x:60,y:20},{x:30,y:40},{x:90,y:35},{x:20,y:70},{x:50,y:55},{x:80,y:60},{x:40,y:85},{x:70,y:80},{x:100,y:75},{x:15,y:50},{x:105,y:50},{x:60,y:100}][(i-1)].x} y2={[{x:60,y:20},{x:30,y:40},{x:90,y:35},{x:20,y:70},{x:50,y:55},{x:80,y:60},{x:40,y:85},{x:70,y:80},{x:100,y:75},{x:15,y:50},{x:105,y:50},{x:60,y:100}][(i-1)].y} stroke="#F5922B" strokeWidth="0.5" opacity="0.2"/>}
        </React.Fragment>
      ))}
    </svg>
  ),
  photo: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <rect x="30" y="25" width="60" height="75" rx="8" fill="none" stroke="#F5922B" strokeWidth="1.5" opacity="0.6"/>
      <circle cx="60" cy="55" r="15" fill="none" stroke="#F5922B" strokeWidth="1.5" opacity="0.8"/>
      <circle cx="60" cy="55" r="6" fill="#F5922B" opacity="0.4"/>
      {[0,60,120,180,240,300].map((a,i) => <line key={i} x1={60+Math.cos(a*Math.PI/180)*20} y1={55+Math.sin(a*Math.PI/180)*20} x2={60+Math.cos(a*Math.PI/180)*30} y2={55+Math.sin(a*Math.PI/180)*30} stroke="#F5922B" strokeWidth="0.8" opacity="0.3"/>)}
    </svg>
  ),
  vitamins: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <circle cx="35" cy="40" r="12" fill="none" stroke="#F5922B" strokeWidth="1.5" opacity="0.8"/>
      <circle cx="85" cy="40" r="12" fill="none" stroke="#F5922B" strokeWidth="1.5" opacity="0.8"/>
      ...
      <text x="35" y="44" textAnchor="middle" fill="#F5922B" fontSize="8" fontWeight="bold">Fe</text>
      <text x="85" y="44" textAnchor="middle" fill="#F5922B" fontSize="8" fontWeight="bold">Ca</text>
      <text x="60" y="89" textAnchor="middle" fill="#52B044" fontSize="8" fontWeight="bold">D3</text>
    </svg>
  ),
  geo: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      {[{x:30,y:30},{x:70,y:25},{x:100,y:45},{x:20,y:60},{x:55,y:55},{x:85,y:70},{x:35,y:90},{x:75,y:90}].map((p,i) => <circle key={i} cx={p.x} cy={p.y} r="3" fill="#F5922B" opacity="0.2"/>)}
      <circle cx="55" cy="55" r="8" fill="none" stroke="#F5922B" strokeWidth="1.5" opacity="0.9"/>
      <circle cx="55" cy="55" r="3" fill="#F5922B" opacity="0.8"/>
      <circle cx="55" cy="55" r="18" fill="none" stroke="#F5922B" strokeWidth="0.5" opacity="0.2"/>
      <circle cx="55" cy="55" r="30" fill="none" stroke="#F5922B" strokeWidth="0.3" opacity="0.1"/>
    </svg>
  ),
  recalc: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <polyline points="10,80 30,75 50,70 60,85 70,50 90,45 110,40" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="1.5" strokeDasharray="3 2"/>
      <polyline points="10,80 30,75 50,70 60,55 70,50 90,45 110,40" fill="none" stroke="#F5922B" strokeWidth="1.5"/>
      <circle cx="60" cy="85" r="4" fill="rgba(255,255,255,0.3)"/>
      <circle cx="60" cy="55" r="4" fill="#F5922B"/>
      <line x1="60" y1="82" x2="60" y2="58" stroke="#F5922B" strokeWidth="0.8" opacity="0.5" strokeDasharray="2 1"/>
    </svg>
  ),
  shopping: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <circle cx="60" cy="60" r="35" fill="none" stroke="#F5922B" strokeWidth="0.5" opacity="0.2"/>
      <circle cx="45" cy="45" r="15" fill="#F5922B" opacity="0.1" stroke="#F5922B" strokeWidth="0.8"/>
      <circle cx="75" cy="50" r="12" fill="#F5922B" opacity="0.1" stroke="#F5922B" strokeWidth="0.8"/>
      <circle cx="55" cy="75" r="13" fill="#F5922B" opacity="0.1" stroke="#F5922B" strokeWidth="0.8"/>
      <circle cx="45" cy="45" r="3" fill="#F5922B" opacity="0.6"/><circle cx="75" cy="50" r="3" fill="#F5922B" opacity="0.6"/><circle cx="55" cy="75" r="3" fill="#F5922B" opacity="0.6"/>
    </svg>
  ),
  privacy: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <path d="M60 20 L95 40 L95 70 Q95 100 60 110 Q25 100 25 70 L25 40 Z" fill="none" stroke="#F5922B" strokeWidth="1.5" opacity="0.4"/>
      <rect x="50" y="55" width="20" height="16" rx="3" fill="none" stroke="#F5922B" strokeWidth="1.5" opacity="0.8"/>
      <path d="M54 55 L54 48 Q54 42 60 42 Q66 42 66 48 L66 55" fill="none" stroke="#F5922B" strokeWidth="1.5" opacity="0.8"/>
      <circle cx="60" cy="63" r="2" fill="#F5922B"/>
    </svg>
  ),
  fasting: (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      <circle cx="60" cy="60" r="40" fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="1.5"/>
      <path d="M60 20 A40 40 0 0 1 97 73" fill="none" stroke="#F5922B" strokeWidth="4" opacity="0.8" strokeLinecap="round"/>
      ...
      <circle cx="60" cy="60" r="2" fill="#F5922B"/>
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#2D3239", color: "rgba(255,255,255,0.9)" }}>
      ...
      >
        <div className="max-w-[1200px] mx-auto flex justify-between items-center h-16 px-6">
          <Image src="/brand/logo-horizontal-clean.png" alt="Health Code" width={160} height={50} priority style={{ height: 'auto' }} />
          <div className="flex items-center gap-6">
            ...
            <Link href="/variant-a" className="text-sm font-semibold transition-colors" style={{ color: "#F5922B" }}>Начать</Link>
          </div>
        </div>
      </nav>

      {/* ── SCREEN 1: Hero (100vh) ── */}
      <section className="min-h-screen flex flex-col items-center justify-center px-6 relative overflow-hidden" style={{ background: "linear-gradient(145deg, #3a3f47 0%, #2D3239 25%, #23272d 50%, #2D3239 75%, #353a41 100%)" }}>
        ...
        <div className="absolute top-[10%] right-[30%] w-[300px] h-[300px] rounded-full opacity-[0.03]" style={{ background: "radial-gradient(circle, rgba(245,146,43,0.3), transparent 60%)" }} />
        ...
        <div className={`relative z-10 flex flex-col items-center transition-all duration-[2s] ease-out ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <Image
            ...
            alt="Health Code — be more · feel alive"
            width={853}
            height={268}
            priority
      ...
      <section className="py-32 md:py-40 px-6 relative overflow-hidden" style={{ background: "linear-gradient(180deg, #1a1d22 0%, #111318 100%)" }}>
        ...
        <RevealSection>
          <div className="max-w-[900px] mx-auto text-center relative z-10">
            <h2 className="text-3xl md:text-5xl lg:text-6xl font-extrabold leading-tight mb-6" style={{ letterSpacing: "-0.02em" }}>
              Health Code — персональный наставник
              <span className="block" style={{ color: "#F5922B" }}>по питанию, сну и активности</span>
            </h2>
            ...
            <div className="flex items-center justify-center gap-8 md:gap-12">
              {[{ num: "60+", label: "параметров" }, { num: "12", label: "недель" }, { num: "24/7", label: "адаптация" }].map((s, i) => (
                <div key={i} className="text-center">
                  <div className="text-3xl md:text-4xl font-extrabold" style={{ color: "#F5922B" }}>{s.num}</div>
      ...
      <section id="features" className="py-24 px-6">
        <RevealSection>
          <div className="max-w-[1200px] mx-auto">
            <p className="text-center text-sm tracking-[0.25em] uppercase font-medium mb-4" style={{ color: "#F5922B" }}>Уникальные возможности</p>
      ...
      <section className="py-24 px-6">
        <RevealSection>
          <div className="max-w-[800px] mx-auto text-center">
            <p className="text-sm tracking-[0.25em] uppercase font-medium mb-4" style={{ color: "#F5922B" }}>Сравнение</p>
            <h2 className="text-3xl md:text-4xl font-bold mb-4" style={{ letterSpacing: "-0.02em" }}>Трекер калорий vs. Health Code</h2>
            <p className="mb-12" style={{ color: "rgba(255,255,255,0.4)" }}>7 функций, которых нет в обычных приложениях для подсчёта калорий.</p>
            <Image src="/brand/homepage/comparison.png" alt="Health Code vs Calorie Tracker" width={800} height={800} className="w-full" />
          </div>
        </RevealSection>
      </section>
      ...
      <section className="py-24 px-6">
        <RevealSection>
          <div className="max-w-[1000px] mx-auto">
            <p className="text-center text-sm tracking-[0.25em] uppercase font-medium mb-4" style={{ color: "#F5922B" }}>Один день</p>
            <h2 className="text-center text-3xl md:text-4xl font-bold mb-16" style={{ letterSpacing: "-0.02em" }}>Как работает Health Code</h2>
            <div className="relative">
              {/* Vertical line */}
              <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-px" style={{ background: "linear-gradient(180deg, transparent, #F5922B 20%, #F5922B 80%, transparent)" }} />
              {[
                ...
                <div key={i} className={`relative flex items-start gap-6 mb-12 ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} md:text-${i % 2 === 0 ? 'right' : 'left'}`}>
                  <div className="absolute left-6 md:left-1/2 -translate-x-1/2 w-3 h-3 rounded-full" style={{ background: "#F5922B", boxShadow: "0 0 12px rgba(245,146,43,0.5)" }} />
                  <div className={`ml-14 md:ml-0 md:w-1/2 ${i % 2 === 0 ? 'md:pr-12' : 'md:pl-12'}`}>
                    <div className="text-sm font-bold mb-1" style={{ color: "#F5922B" }}>{t.time}</div>
        ...
        <RevealSection className="text-center">
          <div className="max-w-[600px] mx-auto relative z-10">
            <Image
              ...
              style={{ filter: "drop-shadow(0 0 60px rgba(245,146,43,0.3))" }}
            />
            ...
            <Link
              ...
              style={{ background: "linear-gradient(135deg, #E85D04 0%, #F5922B 50%, #FFB347 100%)", color: "#000" }}
            >
              Начать
            </Link>
          </div>
      ...
      <footer className="py-16 px-6" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="max-w-[1000px] mx-auto flex flex-col items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <Image src="/brand/logo-horizontal-clean.png" alt="Health Code" width={160} height={50} style={{ height: 'auto' }} />
          </div>
          ...
          <div className="mt-4 max-w-[600px] text-center p-4 rounded-xl" style={{ background: "rgba(255,255,255,0.02)" }}>
            <p className="text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.15)" }}>
              Health Code — информационный wellness-сервис. Не является медицинской рекомендацией, диагностикой или лечением.
            </p>
          </div>
          <p className="text-xs" style={{ color: "rgba(255,255,255,0.12)" }}>© 2026 Health Code</p>
        </div>
      </footer>
    </div>
  );
