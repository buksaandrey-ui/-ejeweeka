      ...
      <section className="section-padding bg-[var(--bg)] border-t border-[var(--border)]">
        <div className="container mx-auto px-6 max-w-[1000px] reveal">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12">
            <button onClick={() => setActiveStatus('white')} className={`px-6 py-3 rounded-full font-bold text-sm transition-colors ${activeStatus === 'white' ? 'bg-[#D1D5DB] text-black' : 'bg-white/5 text-[#A1A1A6] border border-white/10 hover:bg-white/10'}`}>Хочу попробовать</button>
            <button onClick={() => setActiveStatus('black')} className={`px-6 py-3 rounded-full font-bold text-sm transition-colors ${activeStatus === 'black' ? 'bg-[#9CA3AF] text-black' : 'bg-white/5 text-[#A1A1A6] border border-white/10 hover:bg-white/10'}`}>План на неделю</button>
            <button onClick={() => setActiveStatus('gold')} className={`px-6 py-3 rounded-full font-bold text-sm transition-colors ${activeStatus === 'gold' ? 'bg-[#F5922B] text-black' : 'bg-white/5 text-[#A1A1A6] border border-white/10 hover:bg-white/10'}`}>Хочу максимум</button>
            <button onClick={() => setActiveStatus('family')} className={`px-6 py-3 rounded-full font-bold text-sm transition-colors ${activeStatus === 'family' ? 'bg-[#EF4444] text-white' : 'bg-white/5 text-[#A1A1A6] border border-white/10 hover:bg-white/10'}`}>Для семьи</button>
          </div>

          <div className="max-w-[600px] mx-auto min-h-[200px]">
            {activeStatus === 'white' && (
              <div className="p-8 bg-[#111827] border border-[#374151] rounded-3xl text-center transition-all">
                <h3 className="text-white text-2xl font-bold mb-4">Статус White</h3>
                <p className="text-[#9CA3AF] mb-6">Первый план и базовое понимание, как Health Code работает под твой профиль.</p>
                <div className="font-bold text-white text-xl">0 ₽</div>
              </div>
            )}
            {activeStatus === 'black' && (
              <div className="p-8 bg-[#111827] border border-[#4B5563] rounded-3xl text-center transition-all">
                <h3 className="text-white text-2xl font-bold mb-4">Статус Black</h3>
                <p className="text-[#D1D5DB] mb-6">Питание, рецепты, покупки, витамины, прогресс и Health Connect.</p>
                <div className="font-bold text-white text-xl">490 ₽ / мес</div>
              </div>
            )}
            {activeStatus === 'gold' && (
              <div className="p-8 bg-[#111827] border-2 border-[#F5922B] rounded-3xl text-center transition-all shadow-[0_0_40px_rgba(245,146,43,0.15)] transform scale-105">
                <h3 className="text-[#F5922B] text-2xl font-bold mb-4">Статус Gold</h3>
                <p className="text-[#D1D5DB] mb-6">Фото-анализ, больше вариантов блюд, тренировки и глубокая коррекция плана.</p>
                <div className="font-bold text-[#F5922B] text-xl">990 ₽ / мес</div>
              </div>
            )}
            {activeStatus === 'family' && (
